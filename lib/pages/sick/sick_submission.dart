...
import 'package:superviso/pages/sick/add_sick_submission.dart';
import 'package:superviso/pages/sick/detail_sick_submission.dart';
import 'package:superviso/pages/sick/edit_sick_submission.dart';
import 'package:superviso/repositories/employee.dart';

import 'package:superviso/repositories/sick.dart';

class SickSubmissionPage extends StatefulWidget {
  @override
  _SickSubmissionPageState createState() => _SickSubmissionPageState();
}

class _SickSubmissionPageState extends State<SickSubmissionPage> {
  var status = "pending";
  List sicks = [1, 2, 3];
  var employeeId = 0;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      floatingActionButton: FloatingActionButton.extended(
        ...
        onPressed: () {
          moveToAddSickSUbmission();
          // Get.to(AddSickSubmissionPage());
          // Navigator.push(
          //     context,
          //     PageTransition(
          //         type: PageTransitionType.rightToLeft,
          //         child: AddSickSubmissionPage()));
        },
        icon: Icon(Icons.add),
        label: Text(
          "Sakit",
      ...
      body: SmartRefresher(
        ...
        child: SingleChildScrollView(
          child: Center(
            child: Container(
              child: sicks.length > 0
                  ? Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: <Widget>[
                        // _info(),
                        Container(
                            child: StreamBuilder<List<SickModel>>(
                          stream: fetchSicks(),
                        ...
                        // Container(
                        //     child: Column(
                        //         children: List.generate(sicks.length, (index) {
                        //   return _sicks();
                        // })))
                      ],
                    )
                  : Center(
                      child: Container(
                        margin: EdgeInsets.only(
                            top: Get.mediaQuery.size.height / 2 - 150),
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.center,
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: <Widget>[
  ...
  ///function
  // Stream<List<SickModel>> fetchAttendance() async* {
  //   var ohList = await SickRepository.getAttendances();
  //   yield ohList;
  // }
  ...
  Stream<List<SickModel>> fetchSicks() async* {
    var ohList = await EmployeeRespository().sicks(employeeId);

    yield ohList;
  }

  void moveToAddSickSUbmission() async {
  ...
  void moveToEditSickSUbmission(
      date, attachment, sickDates, note, id, dates) async {
    var result = await Navigator.push(
        context,
        PageTransition(
            type: PageTransitionType.rightToLeft,
            child: EditSickSubmissionPage(
              id: id.toString(),
              note: note,
              date: date,
    ...
    var result = await showModalBottomSheet(
        ...
        builder: (context) {
          return FractionallySizedBox(
              heightFactor: 0.5, child: _bottomSheet(date, dates, id));
        });
    if (result == "update") {
      getDataPref();
    }
  ...
  Widget _sicks(data, index) {
    var d = [];
    //       var d=data[index].sickDates!.split(",");
    for (var date in data[index].sickDates!.split(',')) {
      // _addAndPrint(age);
      d.add(
    ...
    return Container(
      ...
      child: InkWell(
        ...
        child: Card(
          child: InkWell(
            onTap: () {
              // print("tes ${data[index].approvalFlows.toString()}");
              Navigator.push(
                  context,
                  PageTransition(
                      type: PageTransitionType.rightToLeft,
                      child: SickDetailPage(
                          sickDates: "${d}",
                              ...
                              child: Column(
                                children: <Widget>[
                                  ...
                                  Container(
                                    ...
                                    child: BlocBuilder<SickBloc, SickState>(
                                      ...
                                      return state.isLoading == true
                                          ...
                                          : Container(
                                              ...
                                              child: ElevatedButton(
                                                  onPressed: () async {
                                                    context.read<SickBloc>().add(
                                                        DeleteSickSickSubmission(
                                                            id.toString(),
                                                            context));
  ...
  Widget _btnSick(date, attachment, sickDates, note, id, dates) {
    return Expanded(
      child: Container(
        ...
        child: Row(
          ...
          children: <Widget>[
            InkWell(
              onTap: () {
                moveToEditSickSUbmission(
                    date, attachment, sickDates, note, id, dates);
                // Navigator.push(
                //     context,
                //     PageTransition(
                //         type: PageTransitionType.rightToLeft,
